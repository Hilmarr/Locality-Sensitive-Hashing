...
void calculate_hash_values_and_closeToHP(
    ...
    int nPlanes, float* hyperplanes,
    // output
    int* indexGroupMap, bool* closeToHP);
 ...
 * @param closeToHP : For every table and point and hyperplane, there is a boolean
 *                    which is set to true if the point was particularly close
 *                    to the hyperplane when calculating the hash value.
 *                    In this case we will also look on the other side of the hyperplane.
 * 
...
int main() {
    ...
    bool* closeToHP = (bool*)malloc(numTables * nPoints2 * nPlanes * sizeof(bool));
    ...
    calculate_indexGroupMap_and_closeToHP(vectorLength, numTables, nPoints2, points2,
                            nPlanes, hyperplanes, indexGroupMapTableLen, indexGroupMap, closeToHP);
    ...
    int potentialMatchesMaxLen = nPoints1 * 128;
    ...
    int* indexGroupMap, bool* closeToHP)
{
    ...
    for (int table = 0; table < numTables; table++) {
        float* hyperplanes2 = hyperplanes + table * hyperplanesTableLen;
        int* indexGroupMap2 = indexGroupMap + table * indexGroupMapTableLen;
        bool* closeToHP2 = closeToHP + table * closeToHPTableLen;
        // - Match points -
...
{
    const int groupArrayTableLen = nPoints1;
    const int closeToHPTableLen = nPlanes * nPoints2;
