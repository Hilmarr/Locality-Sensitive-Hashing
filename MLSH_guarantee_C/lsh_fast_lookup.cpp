    ...
    int** __restrict__ pGroupIndexMap)
{
    ...
    for (int i = 0; i < nPoints; i++) {
        // fprintf(stderr, "i = %d\n", i);
        int kStart = idxGroupMapExtIndices[i];
        int kEnd = idxGroupMapExtIndices[i+1];
        for (int k = kStart; k < kEnd; k++) {
            // fprintf(stderr, "idxGroupMapExt[%d] = %d\n", k, idxGroupMapExt[k]);
            groupSizeMap[idxGroupMapExt[k]]++;
        }
    }
...
void match_points(int nQueryVecs,
    ...
    for (int i = 0; i < nQueryVecs; i++) {
        ...
        for (int j = jStart; j < jEnd; j++) {
            ...
            // fprintf(stderr, "Diff calculated\n");
        ...
        // fprintf(stderr, "matching %d done\n", i);
...
int main(int argc, char** argv)
{
    ...
    calculate_hash_values_and_dists(nBaseVecs, baseVecs, nPlanes, hyperplanes,
                          indexGroupMap, sqrdDists);

    int* idxGroupMapExt;
    int* idxGroupMapExtIndices;

    find_nearby_boxes(nPlanes, nGroups, nBaseVecs,
                      indexGroupMap, sqrdDists,
